...
$(find "$ROOT" -maxdepth 6 \( -name "*.xcodeproj" -o -name "*.xcworkspace" \) -not -path "$ROOT/.t3/*" -not -path "$ROOT/Pods/*" -not -path "$ROOT/node_modules/*" -not -path "$ROOT/DerivedData/*" -not -path "*/DerivedData/*" 2>/dev/null | sort)
EOF

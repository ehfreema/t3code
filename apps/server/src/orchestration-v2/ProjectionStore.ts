  ...
  Effect.gen(function* () {
    ...
    const selectShellThreadRows = (threadId?: ThreadId) =>
      sql<ShellThreadRow>`
            ...
            WHERE t.thread_id IN (SELECT shell_thread_ids.thread_id FROM shell_thread_ids)
      ...
      sql
        .withTransaction(
          ...
          `,
            ...
            const shells = states
              .filter((state) => state.thread.deletedAt === null)
              .map((state) =>
                shellFromState({
                  state,
      ...
      const service: ProjectionStoreV2Shape = {
        ...
        getShellSnapshot: () =>
          Effect.gen(function* () {
            ...
            const shells = [...existing.replayState.projections.keys()]

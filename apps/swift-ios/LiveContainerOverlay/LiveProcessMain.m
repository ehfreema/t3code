...
int LiveProcessMain(int argc, char *argv[]) {
    ...
    // T3: receive the imported signing certificate from the host app so the iOS 26 JITLess
    // certificate check and dyld hooking work inside the extension sandbox.
    NSData *certificateData = appInfo[@"certificateData"];
    NSString *certificatePassword = appInfo[@"certificatePassword"];
    if (certificateData) {
        [lcUserDefaults setObject:certificateData forKey:@"LCCertificateData"];
    }
    if (certificatePassword) {
        [lcUserDefaults setObject:certificatePassword forKey:@"LCCertificatePassword"];
    }

...
- (instancetype)initWithBundleId:(NSString*)bundleId dataUUID:(NSString*)dataUUID delegate:(id<AppSceneViewControllerDelegate>)delegate {
    ...
    // T3: the LiveProcess extension runs in its own sandbox and cannot read the main app's
    // defaults or the (possibly ungranted) app group. Hand the imported signing certificate
    // through the extension request so the extension can pass the iOS 26 JITLess check.
    NSData *certificateData = [LCUtils certificateData];
    NSString *certificatePassword = LCSharedUtils.certificatePassword;
    if (certificateData) {
        [userInfo setValue:certificateData forKey:@"certificateData"];
    }
    if (certificatePassword) {
        [userInfo setValue:certificatePassword forKey:@"certificatePassword"];
    }

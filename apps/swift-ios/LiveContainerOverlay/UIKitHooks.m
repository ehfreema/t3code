...
void hook_FBScene_performUpdateWithoutActivation(FBScene* self, SEL _cmd, void (^updateBlock)(UIMutableApplicationSceneSettings *, FBSSceneTransitionContext *)) {
    ...
    id wrappedBlock = ^(UIMutableApplicationSceneSettings *settings, FBSSceneTransitionContext *context) {
        ...
        // T3: the safe-area rewrite exists for scaled guest windows (multitask) and the
        // glass-landscape fix. For an unscaled scene (the main T3 Code Live window)
        // keep UIKit's own safe area untouched: overriding it during navigation
        // transitions reads the hosting superview's insets at a transient moment,
        // which flickers the top safe area and visibly shifts the whole top bar.
        BOOL isScaled = fabs(transform.a - 1.0) > 0.001 || fabs(transform.d - 1.0) > 0.001;
        BOOL isGlassLandscape = LCHasRemoteSheetProviderSelector
            && UIInterfaceOrientationIsLandscape(settings.interfaceOrientation);
        if (!isScaled && !isGlassLandscape) {
            return;
        }
